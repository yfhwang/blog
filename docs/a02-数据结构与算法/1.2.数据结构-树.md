...
多叉树，平衡M路查找树。M为平衡树的阶数，表示一个树节点最多有多少个查找路径。

## B树

![997909-20190727111522935-981534929](../images/997909-20190727111522935-981534929.png)

### 特点

- 每个非叶子节点（除了根）具有至少⌈ m/2⌉子节点，最多有m个子节点，m-1个关键字
- 所有叶子高度一致，不存储信息

## B+树

![997909-20190728114240297-169990922](../images/997909-20190728114240297-169990922.png)

### 特点

- 有n颗子树的非叶子节点有n个关键字，不保存信息，仅作为索引，所有数据保存在叶子节点
- 所有叶子节点构成一个链表
...
- 因为查询不稳定，所以可以让更频繁的查询更快。

  如适用于基于频率的搜索，越频繁query的结点越往根上走，前提是需要对query做统计，而且要对key做一些变化。

### B+树优点

- B+树每次查询都是到叶子为止，性能稳定
- 所有的叶子结点使用链表相连，便于区间查找和遍历。缓存加载时局部性更好
...
1. 一个节点是红色，或黑色（2-3树节点要么是2节点要么是3节点）
2. 根结点是黑的，每个叶子结点是黑的
3. 红节点不能连续（不能有4节点。注意2-3的4节点是临时的）
4. 对每个结点，从该结点到其所有子孙节点，包含相同数目的黑结点
...
CFS调度将可运行的任务放置在红黑树上（一种平衡的、二分搜索树，它的键是基于虚拟运行时间的）。

当一个任务变成可运行时，它被添加到树上，当一个任务变成不可运行时，它从树上被删除。一般来说，得到较少处理时间的任务会偏向树的左侧；得到较多处理时间的任务会偏向树的右侧。

- Nginx Timer事件管理

  > https://blog.csdn.net/Dachao0707/article/details/103795675/

通过红黑树维护所有的timer节点，每次处理定时器时，就从红黑树中取出最小的节点，如果超时就处理。

- Epoll事件块的管理

内核中维护的红黑树，用以挂载eptitem数据结构以保存监听的fd
...
- 情况1：叔节点是红色->将祖父变为红色，向上递归

- 情况2：叔节点是黑色，本身是父节点的左子树 拓扑调整

- 情况3：叔节点是黑色，本身是父节点的右子树；转换为情况2再调整
